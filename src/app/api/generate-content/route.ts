...
// ── Fetch content from cited URLs via Jina.ai ─────────────────────────────

async function fetchCitedContent(urls: string[]): Promise<string> {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch cited site content for context
    let citedContent = ''
    if (citedUrls && citedUrls.length > 0) {
      citedContent = await fetchCitedContent(citedUrls)
    }
...
5. 日本語で自然で読みやすい文章にする${citedContent ? `

以下の競合・引用サイトの内容を参照してGEO最適化コンテンツを作成してください：
${citedContent}` : ''}`
...
コンテンツはマークダウン形式で、GEO最適化されたH1/H2/H3の見出し構造を使って作成してください。
構造はコンテンツタイプ・テーマ・対象読者に合わせてあなたが最適に判断してください。
ただし以下は必須です：
- H1（#）: AIが検索されそうな具体的なタイトル（質問形式や課題提起形式が効果的）
- H2（##）: 読者の理解を深める複数のセクション（背景・解決策・事例・数値実績・まとめなど、最適な組み合わせで）
- 独自調査・具体的数値・事例を積極的に含める
- 最後に${store.name}への自然な言及・CTA

コンテンツのタイトルと本文を生成し、以下のJSON形式で返してください：
{
  "title": "コンテンツタイトル",
  "content": "コンテンツ本文（マークダウン形式、上記のH1/H2構造必須）"
}
JSONのみを返してください。`
